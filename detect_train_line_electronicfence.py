...
def main(image,image_name):
    ...
    with open('/home/cz/project/yolov7-mask-linux/data/hyp.scratch.mask.yaml') as f:
        hyp = yaml.load(f, Loader=yaml.FullLoader)

    weigths = torch.load('/home/cz/project/yolov7-mask-linux/data/yolov7-mask.pt')
    model = weigths['model']
    ...
    if pred is None:
        ...
        target_cls_id = 6  # 设置目标类别
        confidence_threshold = 0.8  # 设置置信度阈值


        for one_mask, bbox, cls, conf in zip(pred_masks_np, nbboxes, pred_cls, pred_conf):
            if cls == target_cls_id and conf > confidence_threshold:
                # 找到满足条件的目标
                color1 = (0, 255, 255)
                pnimg[one_mask] = pnimg[one_mask] * 0.5 + np.array(color1, dtype=np.uint8) * 0.5
                mask_name = f"{target_cls_id}_mask.png"
                mask = one_mask.astype(np.uint8) * 255
                result = electronicfence.fence(mask, image_name)
                ####################################
                # 寻找掩膜轮廓
                contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                # 用红色的线条画出掩膜轮廓
                mask_contours = cv2.drawContours(nimg, contours, -1, (0, 0, 255), 2)
                # 保存结果图片
                # image_name = os.path.splitext(os.path.basename(image_path))[0]

                # 将RGB图像转换为BGR图像
                mask_contours = cv2.cvtColor(np.array(mask_contours), cv2.COLOR_RGB2BGR)
                base64img = img_to_base64.tobase64(mask_contours)

                output_suffix = '_mask.jpg'
                output_path = 'zoutput/{}{}_{}'.format(image_name,str(time.time()), output_suffix)
                if config.image_save:
                    cv2.imwrite(output_path, mask_contours)
                # print(result[1])
                return True, result[0],result[1],result[2],result[3],base64img
        return False, 0, 0, 0, 0, 0
    ...
    #####################################
        ...
        # 记录程序结束时间
        # end_time = time.time()
        # # 计算程序总运行时间（单位：秒）        # total_time = end_time - start_time
        # print("Total time:", total_time, "s")



if __name__ == '__main__':
    image_path = 'zinput/J1525_192.168.1.74.jpg'
    image=cv2.imread(image_path)
    main(image,"Stream 5")
